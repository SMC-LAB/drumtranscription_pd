function evaluation_matrix = test_transcription_weka(midiDir,audioDir, window_size)
...
m_snare(idx2) = [];
idx3 = [false;diff(m_hihat)<(1*(10^-2))]; 
m_hihat(idx3) = [];

%load transcription
bass = importdata(strcat(audioDir,strrep(audiofile, '.wav', ''),'_bassd.txt'), ' ');
snare = importdata(strcat(audioDir,strrep(audiofile, '.wav', ''),'_snared.txt'), ' ');
hihat = importdata(strcat(audioDir,strrep(audiofile, '.wav', ''),'_hihatd.txt'), ' ');
%onset = importdata(strcat(audioDir,strrep(audiofile, '.wav', ''),'_onset.txt'), ' ');


bassd = textread(strcat(audioDir,strrep(audiofile, '.wav', ''),'_bassw.txt'), '%s', 'delimiter', '_');
snared = textread(strcat(audioDir,strrep(audiofile, '.wav', ''),'_snarew.txt'), '%s', 'delimiter', '_');
hihatd = textread(strcat(audioDir,strrep(audiofile, '.wav', ''),'_hihatw.txt'), '%s', 'delimiter', '_');
form_data(bassd,strcat(audioDir,strrep(audiofile, '.wav', ''),'_wbass.txt')); 
form_data(snared,strcat(audioDir,strrep(audiofile, '.wav', ''),'_wsnare.txt')); 
form_data(hihatd,strcat(audioDir,strrep(audiofile, '.wav', ''),'_whihat.txt')); 
